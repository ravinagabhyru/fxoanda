...
async fn test_end_to_end_trading_workflow() {
    ...
    assert!(create_order_result.is_ok(), "Failed to create market order: {:?}", create_order_result);
    ...
    // Handle both market open and closed scenarios
    match order_response.order_fill_transaction {
        Some(fill_transaction) => {
            // Market is open: Order filled immediately
        ...
        None => {
            // Market is closed: Order created but not filled
            println!("Market is closed: Order was created but not filled immediately");
            ...
            let current_units = updated_positions.positions
                .as_ref()
                .and_then(|positions| positions.iter().find(|p| p.instrument == Some(instrument.clone())))
                .and_then(|pos| pos.long.as_ref())
                .and_then(|long| long.units)
            ...
            // Position should remain unchanged when market is closed
            assert_eq!(current_units, initial_position,
                "Position should remain unchanged when market is closed");
            ...
            if let Ok(pending_orders) = pending_orders_result {
                if let Some(orders) = pending_orders.orders {
                    ...
                    if our_order.is_some() {
                        ...
                        if let Some(order_id) = &create_transaction.id {
                            let _ = CancelOrderRequest::new()
                                .with_account_id(account_id.clone())
                                .with_order_specifier(order_id.clone())
                                .remote(&client).await;
                            
                            println!("Cancelled pending market order");
                        }
                    }
                }
            }

            println!("End-to-end trading workflow completed successfully (market closed scenario)!");
            println!("Initial position: {} units", initial_position);
            println!("Final position (unchanged): {} units", current_units);
        }
    }
}
